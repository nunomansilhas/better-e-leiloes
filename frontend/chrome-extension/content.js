// Better E-Leilões - Content Script
// Enhanced card display for E-Leilões auction site
...
// ====================================
// API (using background script for requests)
// ====================================
...
function getEventFromAPI(reference) {
    ...
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({
            ...
            method: 'GET',
            timeout: CONFIG.API_TIMEOUT_GET
        }, (response) => {
            if (chrome.runtime.lastError) {
                console.error(`❌ API error for ${reference}:`, chrome.runtime.lastError);
                resolve(null);
                return;
            }
...
function scrapeEventViaBackend(reference) {
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({
            ...
            method: 'POST',
            timeout: CONFIG.API_TIMEOUT_SCRAPE
        }, (response) => {
            if (chrome.runtime.lastError) {
                console.error(`❌ Scrape error for ${reference}:`, chrome.runtime.lastError);
                resolve(null);
                return;
            }

            if (response && response.error) {
                console.error(`❌ Scrape failed for ${reference}:`, response.error);
                resolve(null);
                return;
            }
...
async function init() {
    console.log('🚀 Better E-Leilões Chrome Extension v1.0.0');
    ...
    console.log('✅ Better E-Leilões extension ativo!');
}
